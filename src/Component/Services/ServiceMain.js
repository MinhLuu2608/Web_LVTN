...
export default function ServiceMain() {
    ...
    const handleDelete = (id) => {
        if (window.confirm('Bạn có chắc chắn muốn ngừng cung cấp dịch vụ này? Nếu dịch vụ chưa có đơn hàng sẽ bị xoá vĩnh viễn?')) {
            fetch("http://localhost:5199/api/service/" + id + "/0", {
                method: 'PUT',
                header: {
    ...
    const handleRecovery = (id) => {
        if (window.confirm('Bạn có chắc chắn muốn phục hồi dịch vụ này?')) {
            fetch("http://localhost:5199/api/service/" + id + "/1", {
                method: 'PUT',
                header: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
    ...
    React.useEffect(() => {
        fetch("http://localhost:5199/api/service/" + searchLoaiDichVu + "/" + searchTinhTrangDichVu)
